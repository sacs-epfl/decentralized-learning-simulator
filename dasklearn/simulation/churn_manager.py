...
class ChurnManager:
    ...
    def __init__(self, settings: SessionSettings):
        ...
        self.prng = np.random.RandomState(settings.seed)
        self.logger = logging.getLogger(self.__class__.__name__)

    def simulate_node_traces(self, total_nodes, min_online, max_online, oscillation_time, simulation_duration, time_step=1):
        """
        ...
        phases = self.prng.uniform(0, 2 * np.pi, total_nodes)
        ...
        raw_bias = self.prng.beta(2, 5, total_nodes)  # values in [0,1]
